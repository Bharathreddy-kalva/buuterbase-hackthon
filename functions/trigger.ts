// POST /trigger  { type, content, source }
//
// FleetMind's core orchestration, ported to a Butterbase Edge Function:
//  1. Classify the event and pick which domain agents must act.
//  2. Fan out to each relevant agent (finance/hr/logistics/support), each
//     reading its own Butterbase table + EverMind Cloud memory and asking
//     Claude (via the Butterbase AI gateway) for an assessment.
//  3. Synthesize an executive summary and compose a CFO iMessage alert.
//  4. Log the event to the `events` table and flush EverMind sessions.
//
// Returns { event_id, agent_outputs, summary, alert_sent, alert_message }.
...
// ---------------------------------------------------------------------
// Prompts (ported from agents/*/*.py and agents/supervisor/supervisor_agent.py)
// ---------------------------------------------------------------------
...
export default async function handler(req: Request, ctx: any): Promise<Response> {
  ...
  try {
    const result = await runTrigger(ctx, body);
    return json(result);
  } catch (e) {
